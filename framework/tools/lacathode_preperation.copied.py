"""
Now with different data input (see import and fastjet)
...
if not os.path.exists(data_path):
    ...
    process = subprocess.Popen(f"curl -L {data_url} -o {data_path}",
                               shell=True)
    process.wait()
...
if not os.path.exists(extra_bkg_path):
    ...
    process = subprocess.Popen(f"wget {extra_bkg_url} -P {args.indir}",
                               shell=True)
    """
    process = subprocess.Popen(f"curl -L {extra_bkg_url} -o {extra_bkg_path}",
                               shell=True)
    if not os.path.exists(args.indir):
        os.makedirs(args.indir)
    process.wait()
